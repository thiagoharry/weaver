...
the small value of 4 KiB. This will force the user to specify a more
...
the initialization and any temporary allocation that could still
exist.  We invoke function \monoespaco{\_Wtrash} to do this: free
everything in the left (allocated during initialization) and right
(temporary) memory stack passing respectively argument 0 and 1:
...
Knowing this we can save the state in our memory (the left stack,
where we put the more permanent allocations):
...
memory state to how it was before entering in the loop in the left
stack:
...
define now a function to the user use our memory allocation. All
permanent user allocations will be done in the left stack using the
...
For temporary allocations that should not be kept after a single
iteration in our main loop, we can use the following function:
...
We will declare a pointer for both functions in struct \monoespaco{W}
and these function will be invoked as \monoespaco{W.alloc()} and
\monoespaco{W.talloc()} respectivelly:
...
the main loop where the allocation happened exist. After the loop
ends, these allocations are automatically disallocated. What is
allocated with the temporary function will be kept only during the
current iteration in the main loop, but will be disallocated after it,
except if we are loading things from files, which requires us to keep
temporary allocation for more time:
...
textures, we need to add all the relevant headers which define the
signature for their loading functions:

@<Local Headers (weaver.c)@>+=
#include "metafont.h"
@

And we also run all the initialization needed for each one of these
formats:
...
For this we need to call some adequate functions in the right
...
the texture. Both values can also be \monoespaco{NULL} to use default
shader or texture.
...
the \monoespaco{shader} directory and image files with texture should
be in the \monoespaco{images} directory. Both directories will be
searched on \monoespaco{W\_DATA\_DIR} if this macro is

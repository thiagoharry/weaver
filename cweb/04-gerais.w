...
Não precisamos desalocá-la explicitamente, pois podemos confiar que
usaremos esta função de modo que o coletor de lixo saberá lidar com
ela.
...
struct _trie *_new_trie(void){
    int i;
    struct _trie *ret;
    ret = (struct _trie *) Walloc(sizeof(struct _trie));
    if(ret == NULL)
...
@<Trie: Declarações@>+=
void _insert_trie(struct _trie *tree, int type, char *name, ...);
@
    ...
    while(*match != '\0' || *p != '\0'){
        if(*p == '\0'){
            ...
            if(current_prefix -> child[(int) *match] != NULL){
                ...
                current_prefix -> child[(int) *match] = _new_node(match, current_prefix);
                current_prefix = current_prefix -> child[(int) *match];
                break;
            }
        }
            ...
            _split_trie(&current_prefix, p, match);
            break;
        }
        else{
            // Checando ramo atual
...
@<Trie: Funções Estáticas@>+=
struct _trie *_new_node(char *string, struct _trie *parent){
    int i, size = strlen(string);
    struct _trie *ret;
    ret = (struct _trie *) Walloc(sizeof(struct _trie));
    if(ret == NULL)
        goto no_memory_error;
    ret -> string = (char *) Walloc(size + 1);
    if(ret -> string == NULL)
        goto no_memory_error;
...
@<Trie: Funções Estáticas@>+=
void _split_trie(struct _trie **origin, char *divergence, char *remaining_match){
    struct _trie *old_way, *new_way;
    int i;
    old_way = _new_node(divergence, *origin);
    new_way = _new_node(remaining_match, *origin);
    for(i = 0; i < 256; i ++){

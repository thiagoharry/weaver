...
// Shader created by: https://www.shadertoy.com/user/mrange
...
#define TAU             (2.0*PI)
#define RESOLUTION      interface_size
#define TIME            time
...
vec4 plane(vec3 ro, vec3 rd, vec3 pp, vec3 off, float aa, float n) {
  ...
  float rep = 2.0*floor(mix(5.0, 30.0, h2) + 0.5);
...
void main() {
  vec2 q = texture_coordinate;
  vec2 p = -1. + 2. * q;
  p.x *= interface_size.x / interface_size.y;

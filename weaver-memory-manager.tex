...
memory allocated after the last marking. The article shows the results
of running some benchmarks comparing its performance against malloc
from standard library running at Linux, OpenBSD, Windows and in a web
browser using Web Assembly.}
...
execução do programa de modo que depois possa liberar toda a memória
alocada desde a última marcação. Também é comparado o desempenho dele
com o {\it malloc} da biblioteca C padrão rodando em Linux, OpenBSD,
Windows e em um navegador de Internet usando {\it Web Assembly}.}
...
dois. E retornará \monoespac{NULL} se não houver espaço o bastante na
arena:
...
de uma só vez toda a memória alocada com \monoespaco{\_Walloc} desde que
...
mais econômica é usar o \monoespaco{mmap}. Ele é uma função bastante
...
função \monoespaco{\_Wcreate\_arena}. O que a função terá que fazer
...
\subsecao{2.5. A Função ``\_Wdestroy\_arena''}
...
quanto está sendo alocado na variável \monoespaco{left\_allocations}.
...
usada. Na função \monoespaco{\_Walloc}, optamos por pedir qualquer
...
No OpenBSD 6.4 a função \monoespaco{free} não parece executar em tempo
...
desempenho equivalente à \monoespaco{\_Walloc}, indicando que o

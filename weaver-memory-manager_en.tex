...
memory. In \italico{Playstation 3}, for example, any memory position
...
function \monoespaco{\_Wcreate\_arena}. This function is responsible for
...
\subsecao{2.5. The function ``\_Wdestroy\_arena''}
...
\subsecao{2.8. The \_Walloc Function}
...
stack. For this we use the function \monoespaco{\_Wtrash}:
...
function \monoespaco{\_Walloc}, we opted for always use the left stack,
...
for \monoespaco{malloc} and \monoespaco{free} are more performance
...
microsecond. Only \monoespaco{malloc} function from standard library
...
The defferent behaviour in Windws functions indicates the differen
...
functions \monoespaco{\_Walloc} and \monoespaco{\_Wtrash}, because
...
a smaller standard deviation. Here \monoespaco{malloc} running in the
web browser had a better running time in average comparing
with \monoespaco{malloc} running natively.
...
drawback is that we don't have the function \monoespaco{free}. All
...
successive calls to \monoespaco{free}, we can use a
single \monoespaco{\_Wtrash} call to free many allocations at once.

In some cases we can use in
sequentially \monoespaco{\_Wmempoint}, \monoespaco{\_Walloc}
and \monoespaco{\_Wtrash} to simulate a call of \monoespaco{malloc}
followed by \monoespaco{free} when we need a temporary and short lived
allocation. In this case, the results show that calling the three
functions is faster than calling the two (\monoespaco{malloc}
and \monoespaco{free}) in all tested platforms.
...
\referencia{Gregory, J. (2019) ``Game Engine Architecture'', CRC Press, third edition.}

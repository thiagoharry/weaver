...
int main(int argc, char **argv){@/
  ...
  @<Caso de uso 7: Criar novo loop principal@>
END_OF_PROGRAM:
  @<Finalização@>
  return return_value;
}
...
avisos sobre uso de funções \monoespaco{fopen} ao invés
de \monoespaco{fopen\_s}, por exemplo. Isso porque as funções seguras
do Windows não são portáveis e usamos as funções antigas de maneira
controlada. Para suprimir tais avisos, precisamos definir a macro
específica abaixo antes de incluir os demais cabeçalhos.
...
\subsecao{5.4. basename: Obtém o nome de um arquivo dado seu caminho}
...
função para copiar todos os arquivos de dentro de um diretório para
...
função \monoespaco{CreateDirectoryA} entende caminhos passados com o
separador Unix.
...
\subsecao{6.9. arg\_is\_valid\_function: Se o argumento pode ser um nome de função de loop principal}
...
\subsecao{6.10. author\_name: Nome do criador do código}
...
\subsecao{6.11. project\_name: Nome do projeto}
...
\subsecao{7.7. Criar novo loop principal}

...
#endif
  @<API Weaver: Formatos de Interface (Inicialização)@>
...
Para permitirmos os diferentes formatos suportados para interpretar
texturas e arquivos de áudio, precisamos adicionar o cabeçalho para
suas funções de carregamento:
...
E executamos também o código de inicialização específico que alguns
formatos requerem:
...
devem estar no diretório \monoespaco{image}. Ambos os diretórios
...
Isso requer que usemos o cabeçalho de strings:
...
Mas além de criar interfaces de usuário visuais, nosso subsistema de
interface também permite criar interfaces sonoras. Elas funcionam de
maneira semelhante, exigindo apenas que o usuário especifique o
caminho para o arquivo de áudio. Aplicaremos aqui a mesma
simplificação adotada anteriormente: em vez de o usuário precisar
informar o caminho completo, a engine assume que todos os arquivos de
som estão localizados no diretório \monoespaco{sound}, sem que seja
...
E colocamos ambas as funções no namespace da API Weaver:
